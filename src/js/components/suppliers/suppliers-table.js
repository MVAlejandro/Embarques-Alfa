...
export async function renderSuppliersTable(suppliersParam = null) {
    ...
    const resultsText = document.getElementById('suppliers-pages-results');

    // Calcular proveedores de la página actual
    const pageStart = (currentPage - 1) * perPage;
    const pageEnd = pageStart + perPage;
    const suppliers = allSuppliers.slice(pageStart, pageEnd);

    // Limpiar tabla antes de insertar
    tbody.innerHTML = '';

    if (!suppliers || suppliers.length === 0) {
        tbody.innerHTML = `<tr><td class="text-center" colspan="8">No hay proveedores registrados</td></tr>`;
        resultsText.textContent = `Mostrando 0 de ${allSuppliers.length} resultados`;
