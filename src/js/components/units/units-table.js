...
export async function renderUnitsTable(unitsParam = null) {
    ...
    units.forEach(unidad => {
        tbody.innerHTML += 
        `<tr>
            <td class="unit-type p-2 ps-3">${unidad.tipo}</td>
            <td class="unit-name px-3 py-2">${unidad.nombre}</td>
            <td class="unit-license px-3 py-2">${unidad.placas}</td>
            <td class="unit-policy px-3 py-2">#${unidad.numero_poliza}</td>
            <td class="unit-description px-3 py-2">${unidad.descripcion}</td>
            <td class="unit-controls text-pageEnd px-3 py-2 pe-4 d-none" data-trans-only>
